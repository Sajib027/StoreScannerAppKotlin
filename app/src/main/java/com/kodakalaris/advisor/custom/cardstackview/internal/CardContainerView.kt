...
class CardContainerView @JvmOverloads constructor(
    context: Context?,
    ...
    override fun onFinishInflate() {
        ...
        contentContainer = findViewById<View>(R.id.card_frame_content_container) as ViewGroup
        overlayContainer = findViewById<View>(R.id.card_frame_overlay_container) as ViewGroup
    }
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        when (MotionEventCompat.getActionMasked(event)) {
            ...
            MotionEvent.ACTION_CANCEL -> if (parent != null && parent.parent != null) {
                parent.parent.requestDisallowInterceptTouchEvent(false)
            }
    ...
    private fun handleActionUp(event: MotionEvent) {
        if (isDragging) {
            ...
            val point: Point =
                Util.getTargetPoint(motionOriginX, motionOriginY, motionCurrentX, motionCurrentY)
            val quadrant: Quadrant =
                Util.getQuadrant(motionOriginX, motionOriginY, motionCurrentX, motionCurrentY)
            var radian: Double =
                Util.getRadian(motionOriginX, motionOriginY, motionCurrentX, motionCurrentY)
            var degree: Double
    ...
    val percentX: Float
        get() {
            var percent: Float = 2f * (ViewCompat.getTranslationX(this) - viewOriginX) / width
            if (percent > 1) {
                percent = 1f
            }
            if (percent < -1) {
    ...
    val percentY: Float
        get() {
            var percent: Float = 2f * (ViewCompat.getTranslationY(this) - viewOriginY) / height
            if (percent > 1) {
                percent = 1f
            }
            if (percent < -1) {
