...
import com.kodakalaris.advisor.custom.cardstackview.internal.CardContainerView
import com.kodakalaris.advisor.custom.cardstackview.internal.CardContainerView.ContainerEventListener
import com.kodakalaris.advisor.custom.cardstackview.internal.CardStackOption
import com.kodakalaris.advisor.custom.cardstackview.internal.CardStackState
import com.kodakalaris.advisor.custom.cardstackview.internal.Util.toPx
import java.util.*
...
class CardStackView @JvmOverloads constructor(
    ...
    private fun initializeViews() {
        ...
        for (i in 0 until option.visibleCount) {
            val view = LayoutInflater.from(context)
                .inflate(R.layout.card_container, this, false) as CardContainerView
            view.setDraggable(false)
            view.setCardStackOption(option)
            view.setOverlay(
                option.leftOverlay,
                option.rightOverlay,
                option.bottomOverlay,
                option.topOverlay
            )
    ...
    private fun update(percentX: Float, percentY: Float) {
        ...
        for (i in 1 until option.visibleCount) {
            ...
            var currentTranslationY = i * toPx(context, option.translationDiff)
            if (option.stackFrom === StackFrom.Top) {
                currentTranslationY *= -1f
            }
            var nextTranslationY = (i - 1) * toPx(context, option.translationDiff)
            if (option.stackFrom === StackFrom.Top) {
                nextTranslationY *= -1f
            }
            val translationY =
    ...
    private fun performReverse(point: Point?, prevView: View, listener: Animator.AnimatorListener) {
    ...
    private fun performSwipe(point: Point?, listener: Animator.AnimatorListener) {
        topView.animate()
    ...
    private fun performSwipe(
        ...
        set: AnimatorSet,
        overlayAnimatorSet: AnimatorSet?,
    ...
    private fun setSwipeEnabled(isSwipeEnabled: Boolean) {
    ...
    private fun swipe(point: Point?, direction: SwipeDirection?) {
        executePreSwipeTask()
    ...
    init {
        ...
        setSwipeThreshold(
            array.getFloat(
                ...
                option.swipeThreshold
            )
        )
        setTranslationDiff(
            array.getFloat(
                R.styleable.CardStackView_translationDiff,
                option.translationDiff
            )
        )
        setScaleDiff(array.getFloat(R.styleable.CardStackView_scaleDiff, option.scaleDiff))
        setStackFrom(
            StackFrom.values()[array.getInt(
                R.styleable.CardStackView_stackFrom,
                option.stackFrom.ordinal
            )]
        )
        setElevationEnabled(
            array.getBoolean(
                R.styleable.CardStackView_elevationEnabled,
                option.isElevationEnabled
            )
        )
        setSwipeEnabled(
            array.getBoolean(
                R.styleable.CardStackView_swipeEnabled,
                option.isSwipeEnabled
            )
        )
        setSwipeDirection(
            SwipeDirection.from(
                array.getInt(
                    R.styleable.CardStackView_swipeDirection,
                    0
                )
            )
        )
